...
if __name__ == "__main__":
    ...
    for line in codecs.open("../data/rcnn/test_seg.json ", "r", predictor.dataset.CHARSET):
    ...
    with codecs.open("output/predict-{}.txt".format(config.model_name), "w", predictor.dataset.CHARSET) as of:
    ...
    # write a
    import pandas as pd
    a = predictor.dataset.id_to_label_map
    df = pd.DataFrame(columns=["A", "B", "C", "D", "E", "F"])
    for i, predict_prob in enumerate(predict_probs):
        for j in range(0, config.eval.top_k):
            if predict_prob[j] > config.eval.threshold:
                df.loc[i, a[j]] = 1
            else:
                df.loc[i, a[j]] = 0
    df.to_csv("output/RCNN.csv", encoding="utf-8", index=False)